<template>
    <div class="relative inline-block h-7 w-14 border  border-colorBorder rounded-3xl">
        ...
        <span @click="toggleCheck" class="relative inline-block h-full w-full bg-gray-600 rounded-full
        transition-colors duration-300" :class="{ 'bg-primary': isCheck }">
            <span class="absolute h-[1.5rem] w-[1.6rem] bg-gray-300 rounded-full transition-all duration-300 left-1"
                :class="[isCheck ? 'translate-x-6' : 'translate-x-0']" aria-checked="1" role="switch"></span>
        </span>
    </div>
</template>
